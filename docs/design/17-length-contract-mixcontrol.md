...
│                                                                                  │
│  整体进度: ████████░░ 69.2%  |  合同交期: 04-01  |  预计全部完工: 03-29 ✅提前2天 │
│                                                                                  │
│  ┌───────────────┬──────┬────┬──────┬──────┬────────┬────────┬────────┬────────┐│
│  │产品           │材质  │长度│合同量│已产量│进度    │交期    │预计完工│状态    ││
│  ├───────────────┼──────┼────┼──────┼──────┼────────┼────────┼────────┼────────┤│
│  │方管125×125    │Q235B │9m  │20.0T │6.8T  │███░34% │04-01   │03-29   │🟡生产中││
│  │×2.75         │      │    │      │      │        │        │14:00   │提前2天 ││
│  ├───────────────┼──────┼────┼──────┼──────┼────────┼────────┼────────┼────────┤│
│  │方管100×50     │Q235B │6m  │30.0T │30.0T │████100%│03-28   │03-26   │✅已完成││
│  │×4.0          │      │    │      │      │        │        │(实际)  │提前2天 ││
│  ├───────────────┼──────┼────┼──────┼──────┼────────┼────────┼────────┼────────┤│
│  │圆管Φ89×3.5   │Q235B │12m │15.0T │8.2T  │████55% │04-05   │04-02   │🟡生产中││
│  │              │      │    │      │      │        │        │08:00   │提前3天 ││
│  ├───────────────┼──────┼────┼──────┼──────┼────────┼────────┼────────┼────────┤│
│  │合计           │      │    │65.0T │45.0T │███ 69% │—      │03-29   │        ││
│  │              │      │    │      │      │        │        │(最晚项)│        ││
│  └───────────────┴──────┴────┴──────┴──────┴────────┴────────┴────────┴────────┘│
│                                                                                  │
│  预计完工时间计算方式:                                                            │
...
│  · 生产中的: 按当前产线产能和剩余量推算                                           │
│  · 未开工的: 按排产计划时间                                                       │
│  · 合同整体: 取所有行项中最晚的预计完工时间                                       │
│                                                                                  │
│  ┌─── 预计完工明细(方管125×125×2.75 × 9m) ──────────────────────────────────┐   │
│  │                                                                          │   │
│  │  已完成: 6.8T / 20.0T = 34%                                             │   │
│  │  剩余量: 20.0 - 6.8 = 13.2T                                             │   │
│  │  当前产线: 焊管1线, 产能 1.4 T/h                                         │   │
│  │  当前进行中: SCH-PIPE-0201 剩余 0.2T, 预计 03-22 18:00 完成              │   │
│  │  后续排产: SCH-PIPE-0205 (7T, 03-25 08:00~03-26 14:00)                  │   │
│  │           SCH-PIPE-0210 (6.2T, 03-27 08:00~03-29 14:00)                 │   │
│  │  → 预计全部完工: 03-29 14:00 (交期04-01, 提前约2天)                      │   │
│  │                                                                          │   │
│  │  时间线:                                                                 │   │
│  │  03-22    03-24    03-26    03-28    03-30    04-01                      │   │
│  │  ├────────┼────────┼────────┼────────┼────────┤                          │   │
│  │  ████6.8T ░░░░░░░  ████7T  ░░░░░░░  ███6.2T  │交期                     │   │
│  │  已完成    等待原料  排产中   等待     排产中   ▲                          │   │
│  │                                         ▲预计完工                        │   │
...
预计完工时间的计算逻辑:
  ...
  ┌──────────────────────────────────────────────────────────────────────┐
  │                                                                      │
  │  IF 已完成(produced_weight ≥ required_weight):                       │
  │    预计完工 = 最后一笔报工的 end_time (实际完工)                       │
  │                                                                      │
  │  ELSE IF 有正在进行的排产(schedule_status='IN_PROGRESS'):             │
  │    当前排产剩余 = planned_weight - good_weight                       │
  │    当前产线产能 = bas_wc_product_rate.capacity_per_hour               │
  │    当前排产预计完成时间 = NOW + 剩余 / 产能                           │
  │                                                                      │
  │    总剩余 = required_weight - produced_weight                        │
  │    后续排产 = 查该合同+该产品的后续排产单(CONFIRMED/RELEASED)          │
  │    预计完工 = 最后一个排产单的 schedule_end                            │
  │                                                                      │
  │    如果后续排产不足以覆盖总剩余:                                      │
  │      缺口 = 总剩余 - Σ(后续排产.planned_weight)                      │
  │      额外时间 = 缺口 / 默认产能                                      │
  │      预计完工 = 最后排产end + 额外时间 + 安全余量(1天)                │
  │      标记: ⚠ 排产不足, 预计时间含估算                                 │
  │                                                                      │
  │  ELSE (未开工, 未排产):                                               │
  │    预计完工 = 排产计划的 schedule_end                                  │
  │    如果未排产: 预计完工 = NULL, 标记 "未排产"                         │
  │                                                                      │
  │  合同整体预计完工 = MAX(所有产品行的预计完工)                          │
  │                                                                      │
  └──────────────────────────────────────────────────────────────────────┘
```
