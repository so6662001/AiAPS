...
三条工艺路线:
  冷板(分步): 酸洗 → 冷轧 → 罩退 → 平整 → 冷板成品
  冷板(联合): 酸轧联合(PLTCM) → 罩退 → 平整 → 冷板成品
  镀锌板:     酸洗/酸轧 → (罩退可选) → 镀锌线 → 后处理 → 镀锌板成品
```
...
```
酸轧联合机组 (PLTCM = Pickling Line & Tandem Cold Mill):
  
  · 酸洗段 + 冷轧段在一条产线上串联, 不断带连续生产
  · 热轧卷上线后依次经过:
    开卷 → 焊接(头尾接续) → 酸洗槽 → 漂洗 → 烘干 → 冷轧机(4~6机架) → 卷取
  · 中间不出酸洗卷, 不入库 → 减少一次出入库和转运
  · 排产时作为"一道工序"处理, 但内部有两段工艺参数
...
与分步酸洗+冷轧的区别:
  ...
  ├──────────────┼──────────────────────┼──────────────────────┤
  │ 工序数       │ 2道(各自排产)         │ 1道(统一排产)        │
  │ 中间库存     │ 有(酸洗卷入库)        │ 无(连续不断带)       │
  │ 上料/出库    │ 2次(酸洗上料+冷轧上料)│ 1次(PLTCM上料)      │
  │ 报工/入库    │ 2次(酸洗入库+冷轧入库)│ 1次(PLTCM入库)      │
  │ 产能瓶颈     │ 取决于慢的那道        │ 取决于冷轧段(通常)   │
  │ 换卷         │ 各自换卷              │ 焊接连续, 不停机换卷 │
  │ 排产灵活度   │ 高(可分别调度)        │ 低(绑定为一体)       │
  │ 适用         │ 老线/小产能           │ 新线/大产能          │
  └──────────────┴──────────────────────┴──────────────────────┘
```
...
```sql
-- 酸轧联合线的特有参数(扩展 bas_work_center 或独立配置表)
CREATE TABLE bas_pltcm_config (
    config_id           BIGINT IDENTITY(1,1) PRIMARY KEY,
    wc_id               BIGINT        NOT NULL,     -- 工作中心(酸轧联合线)
    
    -- ═══ 整线参数 ═══
    line_type           VARCHAR(20)   NOT NULL DEFAULT 'PLTCM',
                                                    -- PLTCM=酸轧联合
                                                    -- PICKLE_ONLY=纯酸洗线
                                                    -- CRM_ONLY=纯冷轧机
    max_speed_mpm       DECIMAL(10,2) NOT NULL,     -- 最大线速(m/min)
    max_entry_thickness DECIMAL(10,3) NOT NULL,     -- 最大入口厚度(热轧卷, mm)
    min_exit_thickness  DECIMAL(10,3) NOT NULL,     -- 最小出口厚度(冷硬卷, mm)
    max_width           DECIMAL(10,3) NOT NULL,     -- 最大带宽(mm)
    min_width           DECIMAL(10,3) NOT NULL,     -- 最小带宽(mm)
    max_reduction_pct   DECIMAL(8,2)  NOT NULL,     -- 最大压下率(%)
    
    -- ═══ 酸洗段参数 ═══
    pickle_tank_count   INT           NULL,         -- 酸洗槽数量
    pickle_type         VARCHAR(20)   NULL,         -- HCL=盐酸 H2SO4=硫酸
    pickle_speed_mpm    DECIMAL(10,2) NULL,         -- 酸洗段最大速度
    
    -- ═══ 冷轧段参数 ═══
    stand_count         INT           NOT NULL,     -- 机架数: 4/5/6
    roll_change_min     INT           NOT NULL DEFAULT 180, -- 换辊时间(min)
    
    -- ═══ 焊机参数(连续头尾焊接) ═══
    welder_type         VARCHAR(20)   NULL,         -- FLASH=闪光焊 LASER=激光焊
    weld_time_sec       INT           NULL,         -- 焊接时间(秒)
    max_weld_thickness  DECIMAL(10,3) NULL,         -- 焊机最大可焊厚度
    
    -- ═══ 换产参数 ═══
    width_change_min    INT           NOT NULL DEFAULT 15,  -- 宽度切换时间(min)
    thickness_change_min INT          NOT NULL DEFAULT 20,  -- 厚度切换时间(min)
    grade_change_min    INT           NOT NULL DEFAULT 30,  -- 材质切换时间(min)
    
    CONSTRAINT FK_pltcm_wc FOREIGN KEY (wc_id) 
...
排产时的处理方式:
  ...
  1. 工艺路线识别:
     物料有两条可选路线:
     ├── 路线A: 酸洗(工序10) → 冷轧(工序20) — 分步, 2道工序
     └── 路线B: 酸轧联合(工序10) — 合并, 1道工序
     
     系统根据产品规格+产能安排自动选择路线,
     或由计划员手动指定使用哪条线

  2. 排产单结构:
     路线A → 2个排产单(SCH-酸洗 + SCH-冷轧), 中间有入库出库
     路线B → 1个排产单(SCH-PLTCM), 无中间库存

  3. 路线B的排产单:
     ┌────────────────────────────────────────────────────────────┐
     │ schedule_no: SCH-PLTCM-0301                               │
     │ material_id: 冷硬卷 0.5×1250 (产出物料)                   │
     │ raw_material_id: 热轧卷 2.5×1250 (投入物料)               │
     │                                                            │
     │ 工序(aps_schedule_oper):                                   │
     │ ┌────┬──────────┬──────────────┬──────────────────────┐   │
     │ │序号│工序名称   │投入物料      │产出物料              │   │
     │ ├────┼──────────┼──────────────┼──────────────────────┤   │
     │ │ 10 │酸洗段    │热轧卷2.5×1250│(不出卷, 连续传递)    │   │
     │ │ 20 │冷轧段    │(连续接收)    │冷硬卷0.5×1250       │   │
     │ └────┴──────────┴──────────────┴──────────────────────┘   │
     │                                                            │
     │ 特殊标记:                                                  │
     │   is_combined_process = 1  (联合工序, 中间不出入库)         │
     │   combined_process_type = 'PLTCM'                          │
     └────────────────────────────────────────────────────────────┘

  4. 上料/报工:
     · 上料: 只需领1次料(热轧卷), 上到PLTCM入口
     · 换卷: 焊机连续焊接, 前一卷尾接后一卷头, 不停机
       → 换料记录(prd_material_swap)只记录焊接时间点, 不停产
     · 报工: 只报1次(PLTCM整线产出), 投入=热轧卷重, 产出=冷硬卷重

  5. 产能计算:
     PLTCM产能由两段中的瓶颈决定:
     ├── 酸洗段: 取决于酸洗速度和带钢厚度
     └── 冷轧段: 取决于压下率和机架数
     实际线速 = MIN(酸洗段最大速度, 冷轧段最大速度)
     
     小时产能(T/h) = 线速(m/min) × 60 × 带宽(m) × 出口厚度(m) × 密度(T/m³)
```

### 0.5.4 排产表增加联合工序标记

```sql
-- aps_schedule 增加联合工序标记
ALTER TABLE aps_schedule ADD (
    is_combined_process BIT NOT NULL DEFAULT 0,     -- 是否联合工序(酸轧一条线)
    combined_process_type VARCHAR(20) NULL           -- PLTCM=酸轧联合 / 其他联合类型
);

-- aps_schedule_oper 增加内部连续标记
ALTER TABLE aps_schedule_oper ADD (
    is_continuous_pass  BIT NOT NULL DEFAULT 0,     -- 是否连续传递(中间不出卷)
    -- 当 is_continuous_pass=1 时:
    -- · 本工序的产出不入库
    -- · 直接作为下一工序的输入
    -- · 不生成 inv_transaction 出入库流水
    -- · 不生成 inv_receipt 入库单
    pass_to_next_oper   BIT NOT NULL DEFAULT 0      -- 产出是否直传下一工序
);
```
...
├────────┤════════════════════════════════════┤════════════════
│        │ SCH-PLTCM-0301                    │ SCH-PLTCM-0302
│        │ 热卷2.5×1250 Q235B → 冷硬0.5×1250│ 热卷3.0×1000 ...
│        │ 投入22T → 产出21.5T(冷硬卷)      │
│        │ [酸洗段+冷轧段] 连续不断带         │
│        │ 📋HT-0120  线速120m/min          │
│        │ → 罩退(F-04 第1层)               │  ← 产出流向
│        │                                    │
│        │ 焊接记录: 14:30 焊接第2卷(HRC-0205)│  ← 不停机换卷
│        │                                    │

对比分步路线:
酸洗线   ████████████ SCH-PICKLE-0301         (2道工序分开排)
冷轧线                      ████████████ SCH-CRM-0301
                     ↑等待酸洗卷入库出库(约4h损失)
```
...
trc_trace_link 记录:
  ...
  ┌───────────────────────────────────────────────────────────────┐
  │ trace_id: 20                                                  │
  │ source: STOCK, CardNo=CD-HR-0200, ResNo=HRC-0200              │
  │         PrdtID=热轧卷2.5×1250, PATName=Q235B, weight=22.0T   │
  │ process: PLTCM (酸轧联合)                                     │
  │         schedule=SCH-PLTCM-0301                               │
  │         combined_process=true                                  │
  │ target: STOCK, CardNo=CD-CRC-0088, ResNo=CRC-0088             │
  │         PrdtID=冷硬卷0.5×1250, weight=21.5T                   │
  │ contract: HT-2026-0120                                        │
  └───────────────────────────────────────────────────────────────┘

  === 对比路线A (分步) — 有2环 ===
  trace_id: 21 — 热轧卷 →[酸洗]→ 酸洗卷
  trace_id: 22 — 酸洗卷 →[冷轧]→ 冷硬卷
  (中间多了酸洗卷的入库+出库+库存记录)
```
